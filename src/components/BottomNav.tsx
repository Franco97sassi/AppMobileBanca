...
export function BottomNav({ active, onChange }: { active: Screen; onChange: (screen: Screen) => void }) {
  return <View style={styles.nav}>{items.map(item => {
    ...
    return <Pressable key={item.id} onPress={() => onChange(item.id)} style={[styles.item, item.id === 'transfer' && styles.mainItem]} accessibilityRole="tab" accessibilityState={{ selected }}>
